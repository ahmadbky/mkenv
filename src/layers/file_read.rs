    ...
    ReadVarError: From<<V as Layer>::Error>,
{
    ...
    fn try_read_var(&self) -> Result<Self::Output, Self::Error> {
        let path = self.var.try_read_var()?;
...
mod tests {
    ...
    fn assert_not_found() {
        ...
        let res = with_env([(VAR_NAME, "./__test_file_not_found")], || {
            config.try_read_var()
        });
        assert_matches!(
            res,
    ...
    fn assert_file_content() {
        ...
        let res = with_file(FILE_PATH, "hello there", || {
            with_env([(VAR_NAME, FILE_PATH)], || config.try_read_var())
        });
    ...
    fn assert_parsed_path_failed() {
        ...
        let res = with_env([(VAR_NAME, "im not expected")], || config.try_read_var());
        assert_matches!(res, Err(ReadVarError::Other(e)) if is_parse_error(&*e));
    }
}
